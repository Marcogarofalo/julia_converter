	...
	for t in 1:T
		for ig1 in 1:length(g5GI_list)
			for ig2 in 1:length(g5GI_list)
				for j in 1:4
					for k in 1:4
						tmp::Complex{Float64} = (raw_data[1, j, g5GI_list[ig1].col[k], k, g5GI_list[ig2].col[j], t]
												 +
												 (raw_data[2, j, g5GI_list[ig1].col[k], k, g5GI_list[ig2].col[j], t])im) * g5GI_list[ig2].val[j] * g5GI_list[ig1].val[k]
						acc_data[ig1+(ig2-1)*length(g5GI_list), t, 1] += tmp.re
						acc_data[ig1+(ig2-1)*length(g5GI_list), t, 2] += tmp.im
					end
				end
			end
		end
...
function read_LIBE_open(conf::String, acc_data::Array{Float64, 7}, basename::String, L::Int32, T::Int32, masses::Vector{Float64},
	...
	slice::Array{Float64, 3} = Array{Float64, 3}(undef, length(g5GI_list)^2, T, 2)
